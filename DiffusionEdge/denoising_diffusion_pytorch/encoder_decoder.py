...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        self.conv_out = torch.nn.Conv2d(block_in,
                                        ...
                                        stride=1,
                                        padding=1)
        # 新增：log-variance 分支（输出形状跟 conv_out 一样）
        self.conv_logvar = torch.nn.Conv2d(block_in,
                                           out_ch,
                                           kernel_size=3,
                                           stride=1,
                                           padding=1)
        # 可以初始化得保守一点：一开始 logvar ≈ 0（σ²≈1）
        torch.nn.init.zeros_(self.conv_logvar.weight)
        torch.nn.init.zeros_(self.conv_logvar.bias)
    ...
    def forward(self, z):
        ...
        # 均值 / 强度（原来的输出）
        x_mu = self.conv_out(h)
        # 不确定性（log σ²）
        x_logvar = self.conv_logvar(h).clamp(min=-3.0, max=3.0)

        if self.tanh_out:
            x_mu = torch.tanh(x_mu)   # 只对均值做 tanh，一般边缘那边默认 tanh_out=False
...
class AutoencoderKL(nn.Module):
    ...
    def forward(self, input, sample_posterior=True):
        ...
        if sample_posterior:
            ...
            z = posterior.mode()
        # dec, x_logvar = self.decode(z)
        # return dec, posterior, x_logvar
        dec = self.decode(z)
