...
class DDPM(nn.Module):
    ...
    @torch.no_grad()
    def sample(self, batch_size=16, up_scale=1, cond=None, denoise=True, calib5=False, **kwargs):
        image_size, channels = self.image_size, self.channels
        if cond is not None:
            batch_size = cond.shape[0]
        return self.sample_fn((batch_size, channels, image_size[0], image_size[1]),
                              up_scale=up_scale, unnormalize=True, cond=cond, denoise=denoise, **kwargs)

    @torch.no_grad()
    def sample_fn(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False, **kwargs):
...
class LatentDiffusion(DDPM):
    ...
    @torch.no_grad()
    def sample(self, batch_size=16, up_scale=1, cond=None, mask=None, denoise=True,calib5=False, **kwargs):
        # image_size, channels = self.image_size, self.channels
        ...
        if self.cfg.model_name == 'cond_unet8' or self.cfg.model_name == 'cond_unet13':
            ...
            z = self.sample_fn((batch_size, channels, image_size[0]//down_ratio, image_size[1]//down_ratio),
                               up_scale=up_scale, unnormalize=False, cond=cond, denoise=denoise, calib5=calib5, **kwargs)
            aux_out = None
        ...
        if self.scale_by_std:
            ...
            z = z.detach()
        #print(z.shape)
        x_rec, x_logvar = self.first_stage_model.decode(z) # 最后一层
        ...
        return x_rec, x_logvar # 返回

    @torch.no_grad()
    def sample_fn(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False,calib5=False, **kwargs):
        batch, device, total_timesteps, sampling_timesteps, eta, objective = shape[0], \
            self.betas.device, self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta, self.objective
        ...
        # K = -1 * torch.ones_like(img)
        cur_time = torch.ones((batch,), device=device)
        kwargs.setdefault("calib5", calib5)
        ...
        for i, time_step in enumerate(time_steps):
            ...
            if i == time_steps.shape[0] - 1:
                s = cur_time
            if cond is not None:
                pred = self.model(img, cur_time, cond, **kwargs)
            else:
                pred = self.model(img, cur_time)
            # C, noise = pred.chunk(2, dim=1)
            C, noise = pred[:2]
