    ...
    def init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                nn.init.constant_(m.bias, 0.)
    def forward(self, x1, x2, **kwargs):  # x1 for q (conditional input), x2 for k,v
        B, C1, H1, W1 = x1.shape
        _, C2, H2, W2 = x2.shape
        ...
        qg = self.q_lin(qg, **kwargs).reshape(
            B, num_window_q, self.nhead, C1 // self.nhead
        ).permute(0, 2, 1, 3).contiguous()

        kg2 = self.k_lin(kg, **kwargs).reshape(
            B, num_window_k, self.nhead, C1 // self.nhead
        ).permute(0, 2, 1, 3).contiguous()

        vg = self.v_lin(kg, **kwargs).reshape(
            B, num_window_k, self.nhead, C1 // self.nhead
        ).permute(0, 2, 1, 3).contiguous()
...
class RelationNet(nn.Module):
    ...
    def forward(self, cond, feat,**kwargs):
        # cluster = cluster.unsqueeze(0).repeat(feature.shape[0], 1, 1, 1)
        cond = self.input_conv1(cond,**kwargs)
        feat = self.input_conv2(feat,**kwargs)
        for att in self.attentions:
            feat = att(cond, feat,**kwargs)
        return feat
...
class Block(nn.Module):
    ...
    def forward(self, x, scale_shift = None,**kwargs):
...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, time_emb = None,**kwargs):
        ...
        if exists(self.mlp) and exists(time_emb):
            time_emb = self.mlp(time_emb,**kwargs)
            time_emb = rearrange(time_emb, 'b c -> b c 1 1')
            scale_shift = time_emb.chunk(2, dim = 1)

        h = self.block1(x, scale_shift = scale_shift,**kwargs)

        h = self.block2(h,**kwargs)

        if isinstance(self.res_conv, nn.Conv2d):
            # 只有是 Conv2d 的时候才传 calib5 / kwargs
            residual = self.res_conv(x, **kwargs)
        else:
            # Identity 这种就老老实实只传 x
...
class LinearAttention(nn.Module):
    ...
    def forward(self, x,**kwargs):
        b, c, h, w = x.shape
        qkv = self.to_qkv(x,**kwargs).chunk(3, dim = 1)
...
class Unet(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if self.cond_pe:
            self.cond_pos_embedding = MySequential(
    ...
    def forward(self, x, time, mask, x_self_cond = None, **kwargs):
        ...
        hm = self.init_conv_mask(mask, **kwargs)
        # if self.cond_pe:
        #     m = self.cond_pos_embedding(m)
        x = self.init_conv(torch.cat([x, F.interpolate(hm[0], size=x.shape[-2:], mode="bilinear")], dim=1), **kwargs)
        r = x.clone()

        t = self.time_mlp(torch.log(time)/4, **kwargs)

        h = []
        h2 = []
        for j, layer in enumerate(self.projects):
            # print(hm[i].shape)
            hm[j] = layer(hm[j], **kwargs)
        hm2 = []
        for j in range(len(hm)):
            hm2.append(hm[j].clone())
        # hm = []
        # hm2 = []
        for j, ((block1, block2, attn, downsample), relation_layer) \
            ...
            x = relation_layer(hm[j], x, **kwargs)
            ...
            x = downsample(x, **kwargs)
            # m = m_downsample(m)


        # x = x + F.interpolate(hm[-1], size=x.shape[2:], mode="bilinear", align_corners=True)
        x = self.mid_block1(x, t, **kwargs)
        x = self.mid_attn(x, **kwargs)
        x = self.mid_block2(x, t, **kwargs)
        x1 = x + self.decouple1(x, **kwargs)
        x2 = x + self.decouple2(x, **kwargs)
        x = x1
        for (block1, block2, attn, upsample), relation_layer in zip(self.ups, self.relation_layers_up):
            x = torch.cat((x, h.pop()), dim = 1)
            x = block1(x, t, **kwargs)
            x = relation_layer(hm.pop(), x, **kwargs)
            x = torch.cat((x, h.pop()), dim = 1)
            x = block2(x, t, **kwargs)
            x = attn(x, **kwargs)
            x = upsample(x, **kwargs)
        ...
        x2 = self.final_res_block2(x2, t, **kwargs)
        x2 = self.final_conv2(x2, **kwargs)
